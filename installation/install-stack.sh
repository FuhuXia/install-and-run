...
function configure_properties() {
    ...
    # AGGREGATOR
    # CKAN DB username
    aggregator_pg_id_for_ckan=ckan_aggregator_user
    # CKAN DB password
    aggregator_pg_pw_for_ckan=pass
    #CKAN DB name
    aggregator_pg_db_for_ckan=ckan_aggregator
    # PyCSW DB username
    aggregator_pg_id_for_pycsw=ckan_aggregator_user
    # PyCSW DB pass
    aggregator_pg_pw_for_pycsw=pass
    #PyCSW Database Name
    aggregator_pg_db_for_pycsw=pycsw_ckan_default

    # PUBLISHER
    # CKAN DB username
    publisher_pg_id_for_ckan=ckan_node_default
    # CKAN DB password
    publisher_pg_pw_for_ckan=pass
    #CKAN DB name
    publisher_pg_db_for_ckan=ckan_node
    #Datastore DB Username
    publisher_pg_id_datastore=ckan_node_datastore_user
    #Datastore DB pass
    publisher_pg_pw_datastore=pass
    #Datastore Database Name
    publisher_pg_db_for_datastore=datastore_node
...
function setup_env() {
    ...
    AGGREGATOR_DIR=/opt/ngds/aggregator/default
    AGGREGATOR_ETC=/opt/ngds/aggregator/etc/ckan
    PUBLISHER_DIR=/opt/ngds/publisher/default
    PUBLISHER_ETC=/opt/ngds/publisher/etc/ckan
...
ggregator_pg_id_for_ckan=$aggregator_pg_id_for_ckan
aggregator_pg_pw_for_ckan=$aggregator_pg_pw_for_ckan
aggregator_pg_db_for_ckan=$aggregator_pg_db_for_ckan
aggregator_pg_id_for_pycsw=$aggregator_pg_id_for_pycsw
aggregator_pg_pw_for_pycsw=$aggregator_pg_pw_for_pycsw
aggregator_pg_db_for_pycsw=$aggregator_pg_db_for_pycsw

publisher_pg_id_for_ckan=$publisher_pg_id_for_ckan
publisher_pg_pw_for_ckan=$publisher_pg_pw_for_ckan
publisher_pg_db_for_ckan=$publisher_pg_db_for_ckan
publisher_pg_id_datastore=$publisher_pg_id_datastore
publisher_pg_pw_datastore=$publisher_pg_pw_datastore
publisher_pg_db_for_datastore=$publisher_pg_db_for_datastore
...
function install_datastore() {
    ...
    echo "CREATE ROLE $publisher_pg_id_datastore ENCRYPTED PASSWORD '$publisher_pg_pw_datastore' NOSUPERUSER NOCREATEDB NOCREATEROLE INHERIT LOGIN;" > $TEMPDIR/default_datastore_user.sql
    run_or_die sudo -u postgres psql -d postgres -f $TEMPDIR/default_datastore_user.sql
    run_or_die sudo -u postgres createdb -O $publisher_pg_id_for_ckan $publisher_pg_db_for_datastore -E utf-8

    #Update URLs
    $PYENV_DIR/bin/python $CONFIG_UPDATER -f $CKAN_ETC/default/development.ini -k ckan.datastore.write_url -v "postgresql://$publisher_pg_id_for_ckan:$publisher_pg_pw_for_ckan@localhost/$publisher_pg_db_for_datastore"
    $PYENV_DIR/bin/python $CONFIG_UPDATER -f $CKAN_ETC/default/development.ini -k ckan.datastore.read_url -v "postgresql://$publisher_pg_id_datastore:$publisher_pg_pw_datastore@localhost/$publisher_pg_db_for_datastore"
...
function install_postgis() {
    ...
    # AGGREGATOR
    run_or_die sudo -u postgres psql -d $aggregatorpg_db_for_ckan -f /usr/share/postgresql/9.1/contrib/postgis-1.5/postgis.sql
    run_or_die sudo -u postgres psql -d $aggregatorpg_db_for_ckan -f /usr/share/postgresql/9.1/contrib/postgis-1.5/spatial_ref_sys.sql

    echo "ALTER TABLE spatial_ref_sys OWNER TO $aggregatorpg_id_for_ckan;" > $TEMPDIR/grants_on_template_postgis.sql
    echo "ALTER TABLE geometry_columns OWNER TO $aggregatorpg_id_for_ckan;" >> $TEMPDIR/grants_on_template_postgis.sql

    run_or_die sudo -u postgres psql -d $aggregatorpg_db_for_ckan -f $TEMPDIR/grants_on_template_postgis.sql

    # PUBLISHER
    run_or_die sudo -u postgres psql -d $publisher_pg_db_for_ckan -f /usr/share/postgresql/9.1/contrib/postgis-1.5/postgis.sql
    run_or_die sudo -u postgres psql -d $publisher_pg_db_for_ckan -f /usr/share/postgresql/9.1/contrib/postgis-1.5/spatial_ref_sys.sql

    echo "ALTER TABLE spatial_ref_sys OWNER TO $publisher_pg_id_for_ckan;" > $TEMPDIR/grants_on_template_postgis.sql
    echo "ALTER TABLE geometry_columns OWNER TO $publisher_pg_id_for_ckan;" >> $TEMPDIR/grants_on_template_postgis.sql

    run_or_die sudo -u postgres psql -d $publisher_pg_db_for_ckan -f $TEMPDIR/grants_on_template_postgis.sql

    run_or_die sudo -u postgres psql -d $publisher_pg_db_for_datastore -f /usr/share/postgresql/9.1/contrib/postgis-1.5/postgis.sql
    run_or_die sudo -u postgres psql -d $publisher_pg_db_for_datastore -f /usr/share/postgresql/9.1/contrib/postgis-1.5/spatial_ref_sys.sql

    echo "ALTER TABLE spatial_ref_sys OWNER TO $publisher_pg_id_for_ckan;" > $TEMPDIR/grants_on_template_postgis.sql
    echo "ALTER TABLE geometry_columns OWNER TO $publisher_pg_id_for_ckan;" >> $TEMPDIR/grants_on_template_postgis.sql

    run_or_die sudo -u postgres psql -d $publisher_pg_db_for_datastore -f $TEMPDIR/grants_on_template_postgis.sql
...
function configure_ngds() {
    ...
    if [ "$deployment_type" = "node" ]
        then
            ...
            $PYENV_DIR/bin/python $CONFIG_UPDATER -f $deployment_file -s "server:main" -k "port" -v "5001"
...
function deploy_in_webserver() {
    ...
    AGGREGATOR_WSGI_SCRIPT=$CKAN_ETC/default/apache.wsgi

    PUBLISHER_WSGI_SCRIPT=$CKAN_ETC/default/apache.wsgi
...
cat <<EOF
<VirtualHost *:80>
    ServerName central.usgin.org
    WSGIScriptAlias /csw $PYCSW_WSGI_SCRIPT
    ProxyPreserveHost On
    ProxyRequests off
    ProxyPass / site_url:5000/
    ProxyPassReverse / site_url:5000/
</VirtualHost>

<VirtualHost *:80>
    ServerName node.usgin.org
    ProxyPreserveHost On
    ProxyRequests off
    ProxyPass / site_url:5001/
    ProxyPassReverse / site_url:5001/
</VirtualHost>
EOF
}
...
# Upstart job for aggregator instance
cat > $NGDS_SCRIPTS/ngds-aggregator-serve.conf <<EOF
#!/bin/bash
start on runlevel [2345]
stop on runlevel [!2345]
respawn
exec $AGGREGATOR_DIR/bin/paster serve -c $AGGREGATOR_ETC/default/production.ini >>/var/log/apache2/ngds-aggregator-serve.log 2>&1
EOF

sudo chmod 755 $NGDS_SCRIPTS/ngds-aggregator-serve.conf
cp $NGDS_SCRIPTS/ngds-aggregator-serve.conf /etc/init/
service ngds-aggregator-serve start

# Upstart job for publisher instance
cat > $NGDS_SCRIPTS/ngds-publisher-serve.conf <<EOF
#!/bin/bash
start on runlevel [2345]
stop on runlevel [!2345]
respawn
exec $PUBLISHER_DIR/bin/paster serve -c $PUBLISHER_ETC/default/production.ini >>/var/log/apache2/ngds-aggregator-serve.log 2>&1
EOF

sudo chmod 755 $NGDS_SCRIPTS/ngds-publisher-serve.conf
cp $NGDS_SCRIPTS/ngds-publisher-serve.conf /etc/init/
service ngds-publisher-serve start

# Upstart job for celery daemon
cat > $NGDS_SCRIPTS/ngds-celeryd.conf <<EOF
#!/bin/bash
start on runlevel [2345]
stop on runlevel [!2345]
respawn
exec $PUBLISHER_DIR/bin/paster --plugin=ckan celeryd -c $PUBLISHER_ETC/default/production.ini >>/var/log/apache2/ngds-celery.log 2>&1
EOF
...
stop on runlevel [!2345]
respawn
exec $AGGREGATOR_DIR/bin/paster --plugin=ckanext-harvest harvester gather_consumer --config=$AGGREGATOR_ETC/default/production.ini >> /var/log/ckan-central-gather.log 2>&1
EOF
...
stop on runlevel [!2345]
respawn
exec $AGGREGATOR_DIR/bin/paster --plugin=ckanext-harvest harvester fetch_consumer --config=$AGGREGATOR_ETC/default/production.ini >> /var/log/ckan-central-fetch.log 2>&1
EOF
...
# Upstart job for ckan harvester run queue
cat > $NGDS_SCRIPTS/ckan-central-run-harvest.conf <<EOF
#!/bin/bash
start on runlevel [2345]
stop on runlevel [!2345]
respawn
exec $AGGREGATOR_DIR/bin/paster --plugin=ckanext-harvest harvester run --config=$AGGREGATOR_ETC/default/production.ini >> /var/log/ckan-central-run-harvest.log 2>&1
EOF
...
stop on runlevel [!2345]
respawn
exec $AGGREGATOR_DIR/bin/python /opt/ngds/aggregator/bin/default/src/pycsw/csw.wsgi >> /var/log/ckan-pycsw-server.log 2>&1
EOF
...
stop on runlevel [!2345]
respawn
exec $AGGREGATOR_DIR/bin/paster --plugin=ckanext-spatial ckan-pycsw load -p $PRODUCTION_PYCSW_CONFIG >> /var/log/ckan-pycsw-loader.log 2>&1
...
function install_csw_server() {
    ...
    run_or_die $AGGREGATOR_DIR/bin/pip install -e git+https://github.com/geopython/pycsw.git@1.8.0#egg=pycsw
    ...
    run_or_die cp $AGGREGATOR_DIR/src/pycsw/default-sample.cfg $AGGREGATOR_DIR/src/pycsw/default.cfg

    CSW_SERVER_HOME=$AGGREGATOR_DIR/src/pycsw
    CSW_DB_PARAMS=postgresql://$pg_id_for_pycsw:$pg_pw_for_pycsw@localhost/$pg_db_for_pycsw
    PYCSW_CONFIG=$AGGREGATOR_DIR/src/pycsw/default.cfg
    PYCSW_URL=$site_url/csw

    $AGGREGATOR_DIR/bin/python $CONFIG_UPDATER -f $PYCSW_CONFIG -s "server" -k "home" -v "$CSW_SERVER_HOME"
    $AGGREGATOR_DIR/bin/python $CONFIG_UPDATER -f $PYCSW_CONFIG -s "repository" -k "database" -v "$CSW_DB_PARAMS"
    $AGGREGATOR_DIR/bin/python $CONFIG_UPDATER -f $PYCSW_CONFIG -s "server" -k "url" -v "$PYCSW_URL"

    run_or_die ln -s $AGGREGATOR_DIR/src/pycsw/default.cfg $AGGREGATOR_ETC/default/pycsw.cfg

    PRODUCTION_PYCSW_CONFIG=$AGGREGATOR_ETC/default/pycsw.cfg

    # Build tables in PyCSW database
    cd $AGGREGATOR_DIR/src/ckanext-spatial
    run_or_die $AGGREGATOR_DIR/bin/paster --plugin=ckanext-spatial ckan-pycsw setup -p $PYCSW_CONFIG

    # Move PyCSW WSGI script out of the way so that 'create_pycsw_wsgi_script()' can make custom one
    run_or_die mv $AGGREGATOR_DIR/src/pycsw/csw.wsgi $AGGREGATOR_DIR/src/pycsw/csw.wsgi.bak
}
...
function install_aggregator() {

    #Path where application and data are installed.
    APPS=/opt/ngds/aggregator
